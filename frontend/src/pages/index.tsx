...
interface TargetPuttingManagerProps {
	...
	setTargetInfos: (targetInfos: (TargetInfo & { banishThis: () => void })[]) => void;
}
...
const TargetPuttingManager = ({
  ...
  const addTargetInfo = (targetInfo: TargetInfo) => {
    setTargetInfos([...targetInfos, { ...targetInfo, banishThis: () => {
      setTargetInfos(targetInfos.filter(target => target.id !== targetInfo.id));
    } }]);
  }
...
function App() {
	const [targetInfos, setTargetInfos] = useState<(TargetInfo & { banishThis: () => void })[]>([]);
	...
	return (
      ...
      <Physics>
        ...
        <DebugCubeProducer timeSpan={1000} position={new THREE.Vector3(0, 10, 0)} />
			</Physics>
		</Canvas>
	);
}
